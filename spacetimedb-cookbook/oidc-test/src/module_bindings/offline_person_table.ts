...
/**
 * Table handle for the table `offline_person`.
 *
 * Obtain a handle from the [`offlinePerson`] property on [`RemoteTables`],
 * like `ctx.db.offlinePerson`.
 *
 * Users are encouraged not to explicitly reference this type,
 * but to directly chain method calls,
 * like `ctx.db.offlinePerson.on_insert(...)`.
 */
export class OfflinePersonTableHandle {
  ...
  /**
   * Access to the `identity` unique index on the table `offline_person`,
   * which allows point queries on the field of the same name
   * via the [`OfflinePersonIdentityUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.offlinePerson.identity().find(...)`.
   *
   * Get a handle on the `identity` unique index on the table `offline_person`.
   */
  identity = {
