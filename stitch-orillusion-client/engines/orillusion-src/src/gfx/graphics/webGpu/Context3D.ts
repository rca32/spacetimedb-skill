...
export class Context3D extends CEventDispatcher {
    ...
    async init(canvasConfig?: CanvasConfig): Promise<boolean> {
        ...
        // Chromium on Windows currently ignores powerPreference for WebGPU adapter selection.
        // Avoid passing it there to prevent noisy console warnings.
        const isWindows = typeof navigator !== 'undefined' && /Windows/i.test(navigator.userAgent);
        const adapterOptions: GPURequestAdapterOptions = isWindows
            ? {}
            : {
