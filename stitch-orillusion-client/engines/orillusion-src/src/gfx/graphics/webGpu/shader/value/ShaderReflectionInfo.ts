    ...
    public static combineShaderReflectionVarInfo(shaderReflection: ShaderReflection, shader_variables: ShaderReflectionVarInfo[]) {
        for (const iterator of shader_variables) {
            ...
            if (shaderReflection.groups[iterator.group][iterator.binding]) {
                ...
                let bInfo = iterator;
                const aFields = aInfo.dataFields;
                const bFields = bInfo.dataFields;
                const hasStructData = !aInfo.dataIsBuiltinType || !bInfo.dataIsBuiltinType;
                if (import.meta.env.DEV) {
                    ...
                    if (hasStructData && (!aFields || !bFields)) {
                ...
                const fields: ShaderReflectionStructInfo[] = [];
                if (aFields) {
                    for (let i = 0; i < aFields.length; i++) {
                        const element = aFields[i];
                        fields[i] = element;
                    }
                }
                if (bFields) {
                    for (let i = 0; i < bFields.length; i++) {
                        const element = bFields[i];
                        fields[i] = element;
                    }
                }
                ...
                combineInfo = {
                    ...aInfo,
                    ...bInfo,
                    dataFields: fields.length > 0 ? fields : null,
                };
            }
