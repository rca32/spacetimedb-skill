# Domain: Inventory, Trade, Economy (Web)
...
- 캐릭터 진입 시 `inventory_bootstrap` 호출
- projection 수신 후 인벤토리 UI 초기 구성
...
- 체결: `market_order_match`
- 가격 UI: `price_index`
- 지갑 UI: `player_wallet_view`

## 4. Consistency Rules
- 인벤토리 수량 authoritative 소스는 projection
- `trade_offer`와 인벤토리 projection 교차 검증
...
모든 실패는 toast + 패널 상태 복구로 종료한다.

## 6. Acceptance Criteria
- 스택 이동/분할/병합 100회 반복 무결성 유지
- 거래 성립/취소 후 양측 UI 일관성 유지
- 주문/취소/체결 후 가격/지갑 표시 정상
