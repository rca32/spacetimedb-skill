# Domain: Building, Claim, Housing (Web)
...
5. `build_progress == build_required` 시 `state=1(complete)`
...
UI는 `radius`, `tier` 변화를 실시간 갱신한다.

## 4. Housing Flow
1. `housing_create`
2. `housing_enter`
3. `housing_change_entrance`
4. `interior_mark_empty`
5. `housing_propagate_permissions`
6. `rent_set_whitelist`

## 5. Permission UX
- 권한 부족 시 액션 잠금 + 권한 힌트 노출
- 화이트리스트 편집은 owner/admin만 활성화
- `locked_until` 남은 시간을 HUD에 표시
