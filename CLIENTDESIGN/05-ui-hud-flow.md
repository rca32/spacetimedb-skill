...
3. Character Ready Overlay
4. In-World HUD
5. Modal Panels (Inventory/Trade/Market/Party/Guild/Housing/Quest)
6. Disconnect/Reconnect Overlay
...
## 5. UX Rules
- reducer 실패는 통일된 error toast로 노출
- 연결 단절 시 액션 버튼 비활성화
- 재연결 중 패널은 read-only
- authoritative 반영이 오면 진행바/쿨다운을 서버 값으로 정렬

## 6. Accessibility/Controls
- 키보드+마우스 우선
- 재바인딩 가능한 액션 맵
- 채팅 입력 focus 상태에서 전투 입력 차단
