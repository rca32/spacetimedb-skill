...
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {
    ...
    _table.add_unique_constraint::<String>("permission_key", |row| &row.permission_key);
}
...
/// Access to the `permission_key` unique index on the table `permission_state`,
/// which allows point queries on the field of the same name
/// via the [`PermissionStatePermissionKeyUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.permission_state().permission_key().find(...)`.
pub struct PermissionStatePermissionKeyUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<PermissionState, String>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> PermissionStateTableHandle<'ctx> {
    /// Get a handle on the `permission_key` unique index on the table `permission_state`.
    pub fn permission_key(&self) -> PermissionStatePermissionKeyUnique<'ctx> {
        PermissionStatePermissionKeyUnique {
            imp: self.imp.get_unique_constraint::<String>("permission_key"),
            phantom: std::marker::PhantomData,
        }
    }
}

impl<'ctx> PermissionStatePermissionKeyUnique<'ctx> {
    /// Find the subscribed row whose `permission_key` column value is equal to `col_val`,
    /// if such a row is present in the client cache.
    pub fn find(&self, col_val: &String) -> Option<PermissionState> {
