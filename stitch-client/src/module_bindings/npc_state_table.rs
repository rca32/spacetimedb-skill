...
/// Access to the `npc_id` unique index on the table `npc_state`,
/// which allows point queries on the field of the same name
/// via the [`NpcStateNpcIdUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.npc_state().npc_id().find(...)`.
pub struct NpcStateNpcIdUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<NpcState, u64>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> NpcStateTableHandle<'ctx> {
    /// Get a handle on the `npc_id` unique index on the table `npc_state`.
    pub fn npc_id(&self) -> NpcStateNpcIdUnique<'ctx> {
        NpcStateNpcIdUnique {
...
impl<'ctx> NpcStateNpcIdUnique<'ctx> {
