...
pub fn physics_state_query(filter: &AoiFilter) -> String {
    format!(
        "SELECT * FROM physics_state p WHERE {}",
        filter.region_dimension_clause("p")
    )
}

pub fn correction_stream_query(identity_hex: &str) -> String {
    format!("SELECT * FROM server_correction c WHERE c.identity = 0x{identity_hex}")
}
...
mod tests {
    ...
    #[test]
    fn aoi_stream_query_validates_range() {
        let err = aoi_stream_query(1, 1, 2, 1, -1, 1).expect_err("invalid range");
        assert!(err.contains("min_chunk_x"));

        let sql = aoi_stream_query(1, 2, -1, 1, -2, 2).expect("valid query");
        assert!(sql.contains("FROM aoi_stream"));
        assert!(sql.contains("a.dimension_id = 2"));
    }

    #[test]
    fn correction_query_uses_identity() {
        let sql = correction_stream_query("deadbeef");
