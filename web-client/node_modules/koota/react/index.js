...
import { useEffect as useEffect3, useMemo as useMemo3, useReducer as useReducer3, useRef as useRef3 } from "react";
function useHas(target, trait) {
  const contextWorld = useWorld();
  const [, forceUpdate] = useReducer3((x) => x + 1, 0);
  const memo = useMemo3(() => target ? createSubscriptions2(target, trait, contextWorld) : void 0, [target, trait, contextWorld]);
...
import { useEffect as useEffect4, useMemo as useMemo4, useReducer as useReducer4, useRef as useRef4 } from "react";
function useTarget(target, relation) {
  const contextWorld = useWorld();
  const [, forceUpdate] = useReducer4((x) => x + 1, 0);
  const memo = useMemo4(() => target ? createSubscriptions3(target, relation, contextWorld) : void 0, [target, relation, contextWorld]);
...
import { useEffect as useEffect5, useMemo as useMemo5, useReducer as useReducer5, useRef as useRef5 } from "react";
function useTargets(target, relation) {
  const contextWorld = useWorld();
  const [, forceUpdate] = useReducer5((x) => x + 1, 0);
  const memo = useMemo5(() => target ? createSubscriptions4(target, relation, contextWorld) : void 0, [target, relation, contextWorld]);
...
import { useEffect as useEffect6, useMemo as useMemo6, useReducer as useReducer6, useRef as useRef6 } from "react";
function useTrait(target, trait) {
  ...
  const memo = useMemo6(() => target ? createSubscriptions5(target, trait, contextWorld) : void 0, [target, trait, contextWorld]);
  if (memoRef.current !== memo) {
    memoRef.current = memo;
...
import { useEffect as useEffect7, useMemo as useMemo7, useRef as useRef7 } from "react";
function useTraitEffect(target, trait, callback) {
  const contextWorld = useWorld();
  const world = useMemo7(() => isWorld(target) ? target : contextWorld, [target, contextWorld]);
  const entity = useMemo7(() => isWorld(target) ? target[$internal].worldEntity : target, [target]);
