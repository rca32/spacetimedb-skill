...
- 가격 동기화: NPC별/품목별 1일 1회, 지역 가격 밴드 기준으로 상한/하한 적용.
- 퀘스트 생성: 하루 1인당 3개까지.
- NPC 동행: 1인당 1명 제한.
...
- 요약 저장: 8턴 또는 세션 토큰 상한 도달 시 요약 테이블 업데이트.
- 요약 후 동작: 동일 세션 유지 + 요약본으로 컨텍스트 축약.
- 종료 조건: 10분 이상 비활성 또는 명시 종료 시 세션 종료.
...
## 퀘스트 검증 상한(초안)
- 기준: 플레이어 진행도(레벨/티어) + llm_params의 레벨-티어 매핑 테이블.
- 예시 룰: L1-5는 “일반~고급”까지만 요구/보상 허용.
- 초과 제안: 서버에서 즉시 거절하고 안전 응답으로 대체.
- 실패 UX: “NPC가 제안을 거절/보류” 문구로 응답, 퀘스트 생성 없음.
